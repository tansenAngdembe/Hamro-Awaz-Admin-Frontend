...
import { ArrowLeft, Shield, CheckSquare, Square, MoveLeft } from "lucide-react";
// import Loader from "@/components/Loader/Loa   der"; 
import Loader from "../../../components/loader/Loader";
// import { setErrorMessage, showSuccess } from "@/utils/alert";
...
const CreateAccessGroup = () => {
    ...
    const fetchAccessGroup = async () => {
        try {
            const response = await api.get(`/accessGroup/${id}`);
            if (response.data.code === 200) {
                ...
                const roleNames = groupData.accessGroupRoleMaps?.map(
    ...
    const handleSelectGroup = (groupName) => {
        ...
        const allSelected = groupRoleNames.every((name) => selectedRoles.includes(name));

        if (allSelected) {
            setSelectedRoles((prev) => prev.filter((name) => !groupRoleNames.includes(name)));
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await api.post("/accessGroup/create", requestData);
            ...
            setErrorMessage(`Failed to ${isEdit ? "update" : "create"} access group`);
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="border rounded p-4 shadow-sm space-y-4">
                    <div className="flex items-center space-x-2 mb-2">
                        ...
                        <h2 className="font-semibold text-lg">{isEdit ? "Edit Access Group" : "Create New Access Group"}</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            <input
                                ...
                                placeholder="Description of the access group"
                                className="border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-400"
                                required
                            />
                        </div>
                        ...
                        <div className="flex flex-col md:col-span-2">
                            ...
                            <textarea
                                ...
                                placeholder="Enter remarks or reason for creating this access group"
                                rows={3}
                                className="border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-400"
                                required
                            />
                ...
                <div className="border rounded p-4 shadow-sm space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">Choose the roles and permissions for this access group</p>
                        </div>
                        <div className="flex items-center space-x-2">
                            <span className="px-2 py-1 border rounded text-sm">{selectedRoles.length} of {adminRoles.length} selected</span>
                            <button
                                type="button"
                    ...
                    {Object.entries(groupedRoles).map(([groupName, roles], idx) => {
                        ...
                        const groupSelectedCount = groupRoleNames.filter((name) => selectedRoles.includes(name)).length;
                        ...
                        return (
                            <div key={groupName} className="space-y-2">
                                <div className="flex items-center justify-between p-2 bg-gray-100 rounded">
                                    ...
                                    <span className="text-sm bg-gray-200 px-2 py-0.5 rounded">{groupSelectedCount}/{roles.length}</span>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 pl-4">
                                    {roles.map((role) => {
                                        ...
                                        return (
                                            <label
                                                key={role.name}
                                                className={`flex items-start space-x-2 p-2 border rounded cursor-pointer ${isSelected ? "border-green-600 bg-green-50" : "border-gray-300 hover:border-green-400"}`}
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={isSelected}
                                                ...
                                                <div>
                                                    ...
                                                    {/* {role.name !== "NONE" && (
                                                        <code className="text-xs bg-gray-200 px-1 py-0.5 rounded mt-1 inline-block">{role.name}</code>
                                                    )} */}
                                                </div>
                                            </label>
                                        );
                                ...
                                {idx < Object.keys(groupedRoles).length - 1 && <hr className="mt-2 border-gray-300" />}
                            </div>
                        );
                    })}
                </div>
                ...
                <div className="border rounded p-4 shadow-sm flex items-center justify-between">
                    <div className="text-sm text-gray-600">
                        {selectedRoles.length === 0 ? (
                            <span className="text-green-600">Please select at least one role</span>
                        ) : (
                    ...
                    <div className="flex space-x-2">
                        ...
                        >
                            {submitting ? (isEdit ? "Updating..." : "Creating...") : (isEdit ? "Update Access Group" : "Create Access Group")}
                        </button>
                    </div>
                </div>
            </form>
